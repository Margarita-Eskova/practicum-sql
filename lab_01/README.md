...
Дамп базы данных `teacher_data` предоставлен преподавателем и доступен для скачивания по ссылке:

🔗 [Скачать файлы дампа (Яндекс.Диск)](https://disk.yandex.ru/d/p3ga3WZpmAw8-Q)

Файлы из дампа я переместила в папку SQL_dumb, которую сделала общей папкой для виртуальной машины, чтобы соединить мой домашний компьютер и виртуальнцю среду и суметь подключиться с помощью этих файловв к базе данных преподавателя на виртуальной машине

<img width="1277" height="759" alt="{10D456A1-2C68-4F63-AF1B-B37DD6C813B4}" src="https://github.com/user-attachments/assets/9cf91448-eea4-482b-ad2a-c8573bdccbb5" />

### 2. Восстановление базы данных через терминал

Восстановление базы из дампа выполнялось на виртуальной машине командой:
```bash
psql -h localhost -p 5433 -U postgres -d teacher_data < /mnt/sql_dumb/bi_sql_data_student_dump.sql
password: `1`
```
### 3. Просмотр базы данных
В результате чего база данных преподавателя появилась автоматически в дбивере:

<img width="564" height="537" alt="{F2AD3D58-A831-4227-B1CD-834CE458E786}" src="https://github.com/user-attachments/assets/9a760625-7e09-496c-a076-939fb61fa0d0" />
